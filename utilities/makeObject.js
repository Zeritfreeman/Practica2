 ...
 * @example
 *
 * const el = makeObject('h1', { class: 'title' }, ['Hi makeObject'])
 * console.log(el)
 *
 * // <h1 class="title">Hi makeObject</h1>
 */
export default function makeObject(tagName, attributes, children) {
  if (typeof tagName !== 'string') throw new Error('tagName must be an string')

  const el = document.makeObject(tagName)
